...
JSObject? _getAudioContext() {
  ...
  try {
    if (_window.has('AudioContext')) {
      final constructor =
          _window.getProperty('AudioContext'.toJS) as JSFunction;
      _audioCtx = constructor.callAsConstructor();
    } else if (_window.has('webkitAudioContext')) {
      final constructor =
          _window.getProperty('webkitAudioContext'.toJS) as JSFunction;
      _audioCtx = constructor.callAsConstructor();
    }
  } catch (_) {}
  return _audioCtx;
...
void playWebClick() {
  try {
    ...
    freq.callMethod('exponentialRampToValueAtTime'.toJS, 400.toJS,
        (currentTime + 0.015).toJS);

    final gainNode = gain.getProperty('gain'.toJS) as JSObject;
    gainNode.callMethod('setValueAtTime'.toJS, 0.08.toJS, currentTime.toJS);
    gainNode.callMethod('exponentialRampToValueAtTime'.toJS, 0.0001.toJS,
        (currentTime + 0.015).toJS);
...
void playWebPageTurn() {
  try {
    ...
    freq.callMethod('exponentialRampToValueAtTime'.toJS, 180.toJS,
        (currentTime + 0.06).toJS);
