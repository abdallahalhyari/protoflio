...
void main() {
  group('ArchitectureSimulatorBloc Test Suite', () {
    test('initial state defaults to first architecture topic at step 0', () async {
    ...
    test('SimulatorTopicSelected switches topic and resets active step to 0', () async {
      final bloc = ArchitectureSimulatorBloc();
    ...
    test('SimulatorStepSelected updates pipeline execution step', () async {
      ...
      await expectLater(
        bloc.stream,
        emits(predicate<ArchitectureSimulatorState>((state) => state.currentStepIndex == 2)),
      );

      await bloc.close();
    });

    test('SimulatorNextStepRequested and SimulatorPreviousStepRequested step through pipeline', () async {
      final bloc = ArchitectureSimulatorBloc();

      bloc.add(const SimulatorNextStepRequested());
      await expectLater(
        bloc.stream,
        emits(predicate<ArchitectureSimulatorState>((state) => state.currentStepIndex == 1)),
      );
    ...
    test('SimulatorAutoPlayToggled activates and deactivates simulator loop', () async {
      final bloc = ArchitectureSimulatorBloc();

      bloc.add(const SimulatorAutoPlayToggled());
      await expectLater(
        bloc.stream,
        emits(predicate<ArchitectureSimulatorState>((state) => state.isPlaying == true)),
      );

      bloc.add(const SimulatorAutoPlayToggled());
      await expectLater(
        bloc.stream,
        emits(predicate<ArchitectureSimulatorState>((state) => state.isPlaying == false)),
      );

      await bloc.close();
    });

    test('SimulatorResetRequested resets step to 0 and pauses execution', () async {
      final bloc = ArchitectureSimulatorBloc();

      bloc.add(const SimulatorStepSelected(3));
      await expectLater(
        bloc.stream,
        emits(predicate<ArchitectureSimulatorState>((state) => state.currentStepIndex == 3)),
      );
      ...
      await expectLater(
        ...
        emits(predicate<ArchitectureSimulatorState>(
            (state) => state.currentStepIndex == 0 && state.isPlaying == false)),
      );
