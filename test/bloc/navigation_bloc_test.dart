...
void main() {
  ...
  group('NavigationBloc Test Suite', () {
    test('initial state defaults to page 0 with 7 pages and top button hidden',
        () async {
    ...
    test('NavigationPageSelected navigates to target clamped within bounds',
        () async {
      final bloc = NavigationBloc();
      ...
      await expectLater(
        bloc.stream,
        emits(const NavigationState(
            pageIndex: 2, pageCount: 7, showScrollToTop: false)),
      );
      ...
      await expectLater(
        bloc.stream,
        emits(const NavigationState(
            pageIndex: 6, pageCount: 7, showScrollToTop: false)),
      );
      ...
      await expectLater(
        bloc.stream,
        emits(const NavigationState(
            pageIndex: 0, pageCount: 7, showScrollToTop: false)),
      );
    ...
    test(
        'NavigationNextPageRequested and NavigationPrevPageRequested step sequentially',
        () async {
      final bloc = NavigationBloc();
    ...
    test(
        'NavigationUrlHashReceived correctly navigates to matched section index',
        () async {
      final bloc = NavigationBloc();
    ...
    test('NavigationScrollToTopToggled updates showScrollToTop flag', () async {
      ...
      await expectLater(
        bloc.stream,
        emits(const NavigationState(
            pageIndex: 0, showScrollToTop: true, pageCount: 7)),
      );
      ...
      await expectLater(
        bloc.stream,
        emits(const NavigationState(
            pageIndex: 0, showScrollToTop: false, pageCount: 7)),
      );
